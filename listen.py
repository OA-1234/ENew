...
def move_robot():
    global use_pid, left_speed, right_speed,time_to_drive
    ...
    while True:
        ...
        ### pid only runs when robot moves forward or backward. Turning does not use pid
            ...
            if (motion == 'autostop') or (motion == 'autoturning'):
                pibot.value = (left_speed, right_speed) 
                ...
                right_encoder.reset()
                flag_new_pid_cycle = True
            elif (motion == 'stop') or (motion == 'turning'):
                pibot.value = (left_speed, right_speed) 
...
@app.route('/automove')
def automove():
    global left_speed, right_speed, motion,time_to_drive
    left_speed, right_speed = float(request.args.get('left_speed')), float(request.args.get('right_speed'))
    time_to_drive = float(request.args.get('drive_time'))
...
def getEncoderValues():
    ...
    return left_encoder.value,right_encoder.value
